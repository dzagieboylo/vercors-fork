...
case object Check {
  def inOrder(
      check1: => Seq[CheckError],
      check2: => Seq[CheckError],
  ): Seq[CheckError] =
    check1 match {
      case Nil => check2
      case more => more
  ...
  def message(context: Node[_] => HasContext): String =
    Message.messagesInContext((this match {
      ...
      case TypeError(expr, expectedType) =>
        Seq(
          context(expr) ->
            s"Expected the type of this expression to be `$expectedType`, but got ${expr.t}."
        )
      ...
      case TypeErrorExplanation(expr, message) => Seq(context(expr) -> message)
      case GenericTypeError(t, expectedType) =>
        Seq(
          context(t) ->
            s"This type variable refers to a name that is not actually a type."
        )
      case OutOfScopeError(use, ref) =>
        Seq(
          context(use) -> "This usage is out of scope,",
          context(ref.decl) -> "since it is declared here.",
        )
      case ThisOutsideScopeError(use) =>
        Seq(
          context(use) ->
            "`this` may not occur outside the declaration it refers to."
        )
      case ThisInConstructorPre(use) =>
        Seq(
          context(use) ->
            "`this` may not occur in the precondition of a constructor."
        )
      case OutOfWriteScopeError(reason, use, ref) =>
        Seq(
          context(use) -> "This may not be rewritten to, since ...",
          context(reason) ->
            "declarations outside this node must not be altered, and ...",
      ...
      case NotAPredicateApplication(res) =>
        Seq(
          context(res) ->
            "This expression is not a (scaled) predicate application"
        )
      case AbstractPredicate(res) =>
        Seq(
          context(res) ->
            "This predicate is abstract, and hence cannot be meaningfully folded or unfolded"
        )
      case RedundantCatchClause(clause) =>
        Seq(
          context(clause) ->
            "This catch clause is redundant, because it is subsumed by the caught types of earlier catch clauses in this block."
        )
      ...
      case SeqProgInstanceMethodArgs(m) =>
        Seq(
          context(m) ->
            "An instance method in a `seq_prog` cannot have any arguments."
        )
      case SeqProgInstanceMethodBody(m) =>
        Seq(
          context(m) -> "An instance method in a `seq_prog` must have a body."
        )
      case SeqProgInstanceMethodNonVoid(m) =>
        Seq(
          context(m) ->
            "An instance method in a `seq_prog` must have return type `void`."
        )
      case SeqProgInvocation(s) =>
        Seq(
          context(s) -> "Only invocations on `this` and endpoints are allowed."
        )
      case SeqProgReceivingEndpoint(e) =>
        Seq(
          context(e) ->
            s"Can only refer to the receiving endpoint of this statement."
        )
      case SeqProgParticipant(s) =>
        Seq(
          context(s) ->
            s"An endpoint is used in this branch which is not allowed to participate at this point in the program because of earlier branches."
        )
...
case class TypeErrorText(expr: Expr[_], expectedType: String)
    extends CheckError {
  val subcode = "type"
}
case class TypeErrorExplanation(expr: Node[_], message: String)
    extends CheckError {
  val subcode = "type"
}
case class GenericTypeError(t: Type[_], expectedType: TType[_])
    extends CheckError {
  val subcode = "genericType"
}
case class OutOfScopeError[G](use: Node[G], ref: Ref[G, _ <: Declaration[G]])
    extends CheckError {
  val subcode = "outOfScope"
}
case class ThisOutsideScopeError[G](use: ThisDeclaration[G])
    extends CheckError {
...
case class OutOfWriteScopeError[G](
    reason: Node[G],
    use: Node[G],
    ref: Ref[G, _ <: Declaration[G]],
) extends CheckError {
  val subcode = "outOfWriteScope"
}
case class DoesNotDefine(
...
case class SeqProgInstanceMethodNonVoid(m: InstanceMethod[_])
    extends CheckError {
...
case object CheckContext {
  case class ScopeFrame[G](
      decls: Seq[Declaration[G]],
      scanLazily: Seq[Node[G]],
  ) {
    private lazy val declSet = decls.toSet
    private lazy val scannedDeclSet =
      scanLazily.flatMap(ResolveReferences.scanScope(TypeResolutionContext()))
        .toSet

    def contains(decl: Declaration[G]): Boolean =
      declSet.contains(decl) ||
        (scanLazily.nonEmpty && scannedDeclSet.contains(decl))
  }
}
...
case class CheckContext[G](
  ...
  /** In effect toScan is just scanned for LocalDecl's, and these are added to
    * decls. We want to delay this, because the scanning operation is expensive,
    * and for most of the transformation run the declaration is declared
    * directly anyway.
    */
  def withScope(
      decls: Seq[Declaration[G]],
  ...
  def inWriteScope[Decl <: Declaration[G]](ref: Ref[G, Decl]): Boolean =
    !undeclared.exists(_.contains(ref.decl)) && scopes.drop(roScopes)
      .exists(_.contains(ref.decl))

  def checkInScope[Decl <: Declaration[G]](
      use: Node[G],
      ref: Ref[G, Decl],
  ): Seq[CheckError] =
    if (inScope(ref))
  ...
  def checkInWriteScope[Decl <: Declaration[G]](
      reason: Option[Node[G]],
      use: Node[G],
      ref: Ref[G, Decl],
  ): Seq[CheckError] =
    if (!inScope(ref))
      Seq(OutOfScopeError(use, ref))
    else if (!inWriteScope(ref))
      Seq(OutOfWriteScopeError(reason.get, use, ref))
    else
      Nil
}

case class UnreachableAfterTypeCheck(message: String, at: Node[_])
