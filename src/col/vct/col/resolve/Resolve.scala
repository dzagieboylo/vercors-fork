...
case object Resolve {
  ...
  def extractLiteral(e: Expr[_]): Option[String] =
    e match {
      case JavaStringValue(guardName, _) => Some(guardName)
  ...
  def isBip(node: Node[_], `type`: String): Boolean =
    node match {
      case JavaAnnotation(JavaTClass(r, _), _) => isBip(r.decl, `type`)
      case c: JavaClassOrInterface[_] =>
        c.modifiers
          .contains(JavaBipAnnotation[InitialGeneration]()(DiagnosticOrigin)) &&
        c.name == `type`
      case _ => false
    }
...
case object ResolveTypes {
  ...
  def resolve[G](
      ...
      javaClassPath: Seq[JavaClassPathEntry],
  ): Seq[GlobalDeclaration[G]] = {
    val ctx = TypeResolutionContext[G](
      externalJavaLoader = externalJavaLoader,
  ...
  ): Seq[Referrable[G]] /* importable? */ =
    imp match {
      case imp @ JavaImport(
            /* static = */ true,
            JavaName(fullyQualifiedTypeName :+ staticMember), /* star = */ false,
          ) =>
        val staticType = Java.findJavaTypeName(fullyQualifiedTypeName, ctx)
          .getOrElse(
            throw NoSuchNameError(
              "class",
              fullyQualifiedTypeName.mkString("."),
              imp,
            )
          )
        Seq(Java.findStaticMember(staticType, staticMember).getOrElse(
          throw NoSuchNameError(
            "static member",
            (fullyQualifiedTypeName :+ staticMember).mkString("."),
            imp,
          )
        ))
      case imp @ JavaImport(
            /* static = */ true,
            JavaName(fullyQualifiedTypeName), /* star = */ true,
          ) =>
  ...
  def enterContext[G](
    ...
    node match {
      ...
      case ns: JavaNamespace[G] =>
        ...
        ctxWithNs.copy(stack =
          (ns.declarations.flatMap(Referrable.from) ++
            ns.imports.flatMap(scanImport(_, ctxWithNs))) +: ctx.stack
        )
      case Scope(locals, body) =>
        ctx.copy(stack =
          ((locals ++ scanScope(ctx)(body)).flatMap(Referrable.from)) +:
            ctx.stack
        )
      case decl: Declarator[G] =>
        ctx
          .copy(stack = decl.declarations.flatMap(Referrable.from) +: ctx.stack)
      case _ => ctx
    }
  ...
  def resolveOne[G](node: Node[G], ctx: TypeResolutionContext[G]): Unit =
    node match {
      ...
      case t @ JavaTClass(ref, _) => ref.tryResolve(name => ???)
      case t @ CTypedefName(name) =>
        t.ref = Some(
          C.findCTypeName(name, ctx)
            .getOrElse(throw NoSuchNameError("typedef", name, t))
        )
      ...
      case t @ TAxiomatic(ref, _) =>
        ref.tryResolve(name =>
          Spec.findAdt(name, ctx)
            .getOrElse(throw NoSuchNameError("adt", name, t))
        )
      case t @ SilverPartialTAxiomatic(ref, partialTypeArgs) =>
        ...
        partialTypeArgs.foreach(mapping =>
          mapping._1.tryResolve(name =>
            Spec.findAdtTypeArg(ref.decl, name)
              .getOrElse(throw NoSuchNameError("type variable", name, t))
          )
        )
      case local: JavaLocal[G] =>
        Java.findJavaName(local.name, fromStaticContext = false, ctx) match {
          case Some(
                _: RefVariable[G] | _: RefJavaField[G] |
                _: RefJavaLocalDeclaration[G] | // Regular names
      ...
      case endpoint: PVLEndpoint[G] =>
        endpoint.cls.tryResolve(name =>
          PVL.findTypeName(name, ctx) match {
...
case object ResolveReferences extends LazyLogging {
  def resolve[G](
    ...
    resolve(
      program,
      ReferenceResolutionContext[G](jp, lsp, importedDeclarations),
    )
  }

  def resolve[G](
      node: Node[G],
      ctxIn: ReferenceResolutionContext[G],
  ): Seq[CheckError] = {
    val ctx =
      resolveFlatlyTopDown(node, ctxIn) match {
      ...
      node match {
        case l @ Let(binding, value, main) =>
          val innerCtx = enterContext(node, ctx)
            .withCheckContext(l.enterCheckContext(ctx.checkContext))
          resolve(binding, innerCtx) ++ resolve(value, ctx) ++
            resolve(main, innerCtx)
        case _ =>
          val innerCtx = enterContext(node, ctx)
          node.checkContextRecursor(
            ctx.checkContext,
            { (ctx, node) => resolve(node, innerCtx.withCheckContext(ctx)) },
          ).flatten
      }

    if (childErrors.nonEmpty)
      childErrors
    else {
  ...
  def resolveFlatlyTopDown[G](
      node: Node[G],
      ctx: ReferenceResolutionContext[G],
  ): Either[Seq[CheckError], ReferenceResolutionContext[G]] =
    node match {
      case f: CFunctionDefinition[G] if f.specs.collectFirst {
            case _: CGpgpuKernelSpecifier[G] => ()
          }.isDefined =>
  ...
  def scanScope[G](
    ...
    node match {
      ...
      case CDeclarationStatement(decl)
          if !(ctx.inGpuKernel &&
            decl.decl.specs.collectFirst { case GPULocal() => () }.isDefined) =>
        Seq(decl)
  ...
  def scanBlocks[G](node: ParRegion[G]): Seq[ParBlock[G]] =
    node match {
  ...
  def scanJavaBipGuards[G](
      nodes: Seq[Declaration[G]]
  ): Seq[(Expr[G], JavaMethod[G])] =
    nodes.collect {
      case m: JavaMethod[G] if BipGuard.getName(m).isDefined =>
        (BipGuard.getName(m).get, m)
    }

  def scanJavaBipStatePredicates[G](
      nodes: Seq[JavaModifier[G]]
  ): Seq[(Expr[G], JavaAnnotation[G])] =
    nodes.collect {
      case ann: JavaAnnotation[G] if isBip(ann, "StatePredicate") =>
        (ann.expect("state"), ann)
    }
  ...
  def enterContext[G](
    ...
    (node match {
      case ns: JavaNamespace[G] =>
        ctx.copy(currentJavaNamespace = Some(ns)).copy(stack =
          ns.imports.flatMap(
            ResolveTypes.scanImport[G](_, ctx.asTypeResolutionContext)
          ) +: ctx.stack
        ).declare(ns.declarations)
      case cls: JavaClassOrInterface[G] => {
        val newCtx = ctx.copy(currentJavaClass = Some(cls))
          .copy(currentThis = Some(RefJavaClass(cls))).declare(cls.decls)
          .declareJavaBipGuards(scanJavaBipGuards(cls.decls))
          .declareJavaBipStatePredicates(scanJavaBipStatePredicates(
            cls.modifiers
          ))
      ...
      case cls: Class[G] =>
        ctx.copy(currentThis = Some(RefClass(cls))).declare(cls.declarations)
      ...
      case method: JavaMethod[G] =>
        ctx.copy(currentResult = Some(RefJavaMethod(method)))
          .copy(inStaticJavaContext =
            method.modifiers.collectFirst { case _: JavaStatic[_] => () }
              .nonEmpty
          ).declare(
            method.declarations ++ method.body.map(scanLabels).getOrElse(Nil)
          )
      case fields: JavaFields[G] =>
        ctx.copy(currentInitializerType = Some(fields.t))
          .copy(inStaticJavaContext =
            fields.modifiers.collectFirst { case _: JavaStatic[_] => () }
              .nonEmpty
          )
      case init: JavaSharedInitialization[G] =>
        ctx.copy(inStaticJavaContext = init.isStatic)
      case locals: JavaLocalDeclaration[G] =>
        ctx.copy(currentInitializerType = Some(locals.t))
      case decl: JavaVariableDeclaration[G] =>
        ctx.copy(currentInitializerType =
          ctx.currentInitializerType.map(t =>
            FuncTools.repeat((t: Type[G]) => TArray(t), decl.moreDims, t)
          )
        )
      case arr: JavaNewLiteralArray[G] =>
        ctx.copy(currentInitializerType =
          Some(
            FuncTools.repeat((t: Type[G]) => TArray(t), arr.dims, arr.baseType)
          )
        )
      case init: JavaLiteralArray[G] =>
        ctx.copy(currentInitializerType =
      ...
      case func: CFunctionDefinition[G] =>
        ...
        if (
          func.specs.collectFirst { case _: CGpgpuKernelSpecifier[G] => () }
            .isDefined
        )
      ...
      case func: CGlobalDeclaration[G] =>
        ...
        func.decl.inits.zipWithIndex.foldLeft(ctx.declare(
          func.decl.contract.givenArgs ++ func.decl.contract.yieldsArgs
        )) { case (ctx, (init, idx)) =>
          val info = C.getDeclaratorInfo(init.decl)
          ctx.declare(info.params.getOrElse(Nil)).copy(currentResult =
            info.params.map(_ => RefCGlobalDeclaration(func, idx))
          )
        }
      ...
      case func: CPPGlobalDeclaration[G] =>
        ...
        func.decl.inits.zipWithIndex.foldLeft(ctx.declare(
          ...
          ctx.declare(info.params.getOrElse(Nil)).copy(currentResult =
            info.params.map(_ => RefCPPGlobalDeclaration(func, idx))
          )
        }
      ...
      case Scope(locals, body) =>
        ctx.declare(locals ++ scanScope(ctx.asTypeResolutionContext)(body))
      case app: ContractApplicable[G] =>
        ctx.copy(currentResult =
          Some(
            Referrable.from(app).head
              .asInstanceOf[ResultTarget[G]] /* PB TODO: ew */
          )
        ).declare(app.declarations ++ app.body.map(scanLabels).getOrElse(Nil))
      case app: Applicable[G] =>
        ctx.declare(app.declarations ++ app.body.map(scanLabels).getOrElse(Nil))
      case declarator: Declarator[G] => ctx.declare(declarator.declarations)
      case _ => ctx
  ...
  def resolveFlatly[G](
    ...
    node match {
      case local @ CLocal(name) =>
        local.ref = Some(
          C.findCName(name, ctx)
            .getOrElse(throw NoSuchNameError("local", name, local))
        )
      case local @ CPPLocal(name, arg) =>
        local.ref = Some(
          CPP.findCPPName(name, ctx).headOption
            .getOrElse(throw NoSuchNameError("local", name, local))
        )
      case local @ JavaLocal(name) =>
        val start: Option[JavaNameTarget[G]] =
          if (ctx.javaBipGuardsEnabled) {
        ...
        local.ref = Some(
          start.orElse(
            Java.findJavaName(
              name,
              fromStaticContext = ctx.inStaticJavaContext,
            ...
            if (ctx.topLevelJavaDeref.isEmpty)
              throw NoSuchNameError("local", name, local)
            else
              RefUnloadedJavaNamespace(Seq(name))
          )
        )
      case local @ PVLLocal(name) =>
        local.ref = Some(
          PVL.findName(name, ctx)
            .getOrElse(throw NoSuchNameError("local", name, local))
        )
      ...
      case local @ TVar(ref) =>
        ref.tryResolve(name =>
          Spec.findLocal(name, ctx)
            .getOrElse(throw NoSuchNameError("type variable", name, local))
        )
      case funcOf @ FunctionOf(v, vars) =>
        v.tryResolve(name =>
          Spec.findLocal(name, ctx)
            .getOrElse(throw NoSuchNameError("local", name, funcOf))
        )
        vars.foreach(v =>
          v.tryResolve(name =>
            Spec.findLocal(name, ctx)
      ...
      case deref @ JavaDeref(obj, field) =>
        deref.ref = Some(
          Java.findDeref(obj, field, ctx, deref.blame)
            .getOrElse(throw NoSuchNameError("field", field, deref))
        )
        if (
          ctx.topLevelJavaDeref.contains(deref) &&
          (deref.ref match {
      ...
      case deref @ PVLDeref(obj, field) =>
        deref.ref = Some(
          PVL.findDeref(obj, field, ctx, deref.blame)
            .getOrElse(throw NoSuchNameError("field", field, deref))
        )
      case deref @ Deref(obj, field) =>
        field.tryResolve(name =>
          Spec.findField(obj, name)
            .getOrElse(throw NoSuchNameError("field", name, deref))
        )
      case deref @ ModelDeref(obj, field) =>
        field.tryResolve(name =>
          Spec.findModelField(obj, name)
            .getOrElse(throw NoSuchNameError("field", name, deref))
        )
      case deref @ SilverDeref(_, field) =>
        field.tryResolve(name =>
          Spec.findSilverField(name, ctx)
            .getOrElse(throw NoSuchNameError("field", name, deref))
        )
      ...
      case inv @ CPPInvocation(obj, args, givenMap, yields) =>
        ...
        Spec.resolveGiven(givenMap, inv.ref.get, inv)
        Spec.resolveYields(ctx, yields, inv.ref.get, inv)
      case inv @ GpgpuCudaKernelInvocation(
            name,
            blocks,
            threads,
            smem_bytes,
        ...
        val kernel = C.findCName(name, ctx)
          .getOrElse(throw NoSuchNameError("kernel", name, inv))
      ...
      case inv @ JavaNewClass(args, typeArgs, name, givenMap, yields) =>
        inv.ref = Some(
          Java.findConstructor(name, args)
            .getOrElse(throw NoSuchConstructor(inv))
        )
        Spec.resolveGiven(givenMap, inv.ref.get, inv)
        Spec.resolveYields(ctx, yields, inv.ref.get, inv)
      case inv @ PVLInvocation(
            None,
            method,
            args,
            typeArgs,
      ...
      case inv @ PVLNew(t, typeArgs, args, givenMap, yields) =>
        inv.ref = Some(
          PVL.findConstructor(t, typeArgs, args)
            .getOrElse(throw NoSuchConstructor(inv))
        )
        Spec.resolveGiven(givenMap, inv.ref.get, inv)
        Spec.resolveYields(ctx, yields, inv.ref.get, inv)
      case n @ NewObject(ref) =>
        ref.tryResolve(name =>
          Spec.findClass(name, ctx.asTypeResolutionContext)
            .getOrElse(throw NoSuchNameError("class", name, n))
        )
      case n @ ModelNew(ref) =>
        ref.tryResolve(name =>
          Spec.findModel(name, ctx.asTypeResolutionContext)
            .getOrElse(throw NoSuchNameError("model", name, n))
        )
      case n @ SilverNewRef(target, fields) =>
        target.tryResolve(name =>
          Spec.findLocal(name, ctx)
            .getOrElse(throw NoSuchNameError("local", name, n))
        )
        fields.foreach(
          _.tryResolve(name =>
            Spec.findSilverField(name, ctx)
              .getOrElse(throw NoSuchNameError("field", name, n))
          )
        )
      case inv @ ADTFunctionInvocation(typeArgs, ref, args) =>
        typeArgs match {
          case Some((adt, typeArgs)) =>
            ...
            adt.tryResolve(name =>
              Spec.findAdt(name, ctx.asTypeResolutionContext)
                .getOrElse(throw NoSuchNameError("adt", name, inv))
            )
            ref.tryResolve(name =>
              Spec.findAdtFunction(adt.decl, name)
      ...
      case inv @ SilverPartialADTFunctionInvocation(
            name,
            args,
            partialTypeArgs,
          ) =>
        inv.ref = Some(
          Spec.findAdtFunction(name, ctx)
            .getOrElse(throw NoSuchNameError("function", name, inv))
        )
        partialTypeArgs.foreach(mapping =>
          mapping._1.tryResolve(name =>
            Spec.findAdtTypeArg(inv.adt, name)
              .getOrElse(throw NoSuchNameError("type variable", name, inv))
          )
        )
      ...
      case inv @ InvokeMethod(obj, ref, _, _, _, givenMap, yields) =>
        ref.tryResolve(name =>
          Spec.findMethod(obj, name)
            .getOrElse(throw NoSuchNameError("method", name, inv))
        )
      ...
      case decl: CPPInit[G] => decl.ref = CPP.findDefinition(decl.decl, ctx)

      case goto @ CGoto(name) =>
        goto.ref = Some(
          Spec.findLabel(name, ctx)
            .getOrElse(throw NoSuchNameError("label", name, goto))
        )
      case goto @ Goto(lbl) =>
        lbl.tryResolve(name =>
          Spec.findLabel(name, ctx)
      ...
      case brk @ Break(Some(lbl)) =>
        lbl.tryResolve(name =>
          Spec.findLabel(name, ctx)
            .getOrElse(throw NoSuchNameError("label", name, brk))
        )
      case cont @ Continue(Some(lbl)) =>
        lbl.tryResolve(name =>
          Spec.findLabel(name, ctx)
            .getOrElse(throw NoSuchNameError("label", name, cont))
        )
      case old @ Old(_, Some(lbl)) =>
        lbl.tryResolve(name =>
          Spec.findLabel(name, ctx)
            .getOrElse(throw NoSuchNameError("label", name, old))
        )

      case recv @ Recv(ref) =>
        ref.tryResolve(name =>
          Spec.findSend(name, ctx)
            .getOrElse(throw NoSuchNameError("send statement", name, recv))
        )

      case res @ AmbiguousResult() =>
        res.ref = Some(
          ctx.currentResult.getOrElse(throw ResultOutsideMethod(res))
        )
      case diz @ AmbiguousThis() =>
        diz.ref = Some(ctx.currentThis.getOrElse(throw WrongThisPosition(diz)))

      case proc: ModelProcess[G] =>
        proc.modifies.foreach(
          _.tryResolve(name =>
            Spec.findModelField(name, ctx)
              .getOrElse(throw NoSuchNameError("field", name, proc))
          )
        )
        proc.accessible.foreach(
          _.tryResolve(name =>
            Spec.findModelField(name, ctx)
              .getOrElse(throw NoSuchNameError("field", name, proc))
          )
      ...
      case inv @ ProcessApply(ref, _) => ref.tryResolve(name => ???)
      case inv @ ActionApply(ref, _) => ref.tryResolve(name => ???)

      case atomic @ ParAtomic(invs, _) =>
        invs.foreach(
          _.tryResolve(name =>
            Spec.findParInvariant(name, ctx)
              .getOrElse(throw NoSuchNameError("invariant", name, atomic))
          )
        )
      case barrier @ ParBarrier(block, invs, _, _, _) =>
        block.tryResolve(name =>
          Spec.findParBlock(name, ctx)
      ...
      case ann @ JavaAnnotation(_, _) if isBip(ann, "Transition") =>
        ...
        def extractExpr(s: Option[Expr[_]]): (String, Origin) =
          s match {
            case None => ("true", ann.o)
            case Some(s @ JavaStringValue(data, _)) => (data, s.o)
            case Some(n) =>
              throw MalformedBipAnnotation(
                n,
                "pre- and post-conditions must be string literals",
              )
          }
      ...
      case ann @ JavaAnnotation(_, _) if isBip(ann, "Invariant") =>
        val expr: Expr[G] = ctx.javaParser
          .parse(getLit(ann.expect("value")), ann.expect("value").o)
        resolve(expr, ctx)
      ...
      case ann @ JavaAnnotation(_, _) if isBip(ann, "ComponentType") =>
        ann.data = Some(BipComponent(
          getLit(ann.expect("name")),
      ...
      case ann: JavaAnnotation[G] if isBip(ann, "Port") =>
        val portType: BipPortType[G] =
          ann.expect("type") match {
            ...
            case e =>
              throw MalformedBipAnnotation(
                e,
                "Can be either PortType.enforceable, spontaneous, or internal",
              )
          }
      ...
      case portName @ JavaBipGlueName(
            JavaTClass(Ref(cls: JavaClass[G]), Nil),
            name,
          ) =>
