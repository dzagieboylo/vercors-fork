...
object TFloats {
  ...
  def ieee754_32bit[G](implicit o: Origin = DiagnosticOrigin): TFloat[G] =
    vct.col.ast.TFloat(8, 24)
  def ieee754_64bit[G](implicit o: Origin = DiagnosticOrigin): TFloat[G] =
    vct.col.ast.TFloat(11, 53)
  def C_ieee754_32bit[G](implicit o: Origin = DiagnosticOrigin): TCFloat[G] =
    vct.col.ast.TCFloat(8, 24)
  def C_ieee754_64bit[G](implicit o: Origin = DiagnosticOrigin): TCFloat[G] =
    vct.col.ast.TCFloat(11, 53)
  ...
  def getFloatMax[G](ll: Type[G], rr: Type[G]): Option[FloatType[G]] = {
    ...
    (l, r) match {
      case (l: FloatType[G], r: FloatType[G])
          if r.exponent == l.exponent && r.mantissa == l.mantissa =>
        Some(promoting(l))
      case (l: FloatType[G], r: FloatType[G])
          if l.exponent < r.exponent && l.mantissa < r.mantissa =>
        Some(promoting(r))
      case (l: FloatType[G], r: FloatType[G])
          if r.exponent < l.exponent && r.mantissa < l.mantissa =>
        Some(promoting(l))
      case (l: FloatType[G], r: FloatType[G]) =>
        ??? // Assuming there is only one Float for each exponent, mantissa
      // If one if them needs to be coerced, we coerce and try again
      case (l: FloatType[G], r) =>
        CoercionUtils.getCoercion(r, l).flatMap(c => getFloatMax(l, c.target))
      case (l, r: FloatType[G]) =>
        CoercionUtils.getCoercion(l, r).flatMap(c => getFloatMax(r, c.target))
      case _ => None
    }
  }
}
...
trait FloatTypeImpl[G] {
  ...
  def layoutFloat(implicit ctx: Ctx): Doc =
  ...
  def layoutDouble(implicit ctx: Ctx): Doc =
    Text(ctx.syntax match {
  ...
  override def layout(implicit ctx: Ctx): Doc =
    if (is_ieee754_32bit)
      layoutFloat
    else if (is_ieee754_64bit)
      layoutDouble
    else
      Text(s"??float_${exponent}_${mantissa}??")
}
