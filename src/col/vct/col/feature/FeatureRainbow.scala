...
class FeatureRainbow[G] {
  ...
  def scanFlatly(node: Node[G]): Seq[Feature] =
    Seq(node match {
      ...
      case node: AmbiguousComputationalAnd[G] =>
        return AmbiguousOperators +:
          (if (node.isBoolOp)
             Seq(ComputationalLogicOperator)
           else
             Seq(BitOperators))
      ...
      case node: AmbiguousSubscript[G] =>
        return AmbiguousOperators +:
          (if (node.isPointerOp)
             Seq(Pointers)
           else if (node.isMapOp)
             Seq(AxiomaticLibraryType)
           else if (node.isArrayOp)
             Seq(Arrays)
           else
             Seq(SilverAxiomaticLibraryType))
      ...
      case node: Return[G] =>
        ...
        if (node.result == Void[G]()(DiagnosticOrigin))
          return Seq(Methods)
        else
          return Seq(NonVoidReturn, Methods)
      case node: Branch[G] =>
        node.branches match {
      ...
      case node: TVoid[G] =>
        if (returnTypes.exists(_ eq node))
          return Nil
        else
          AxiomaticLibraryType
      case node: Void[G] =>
        if (returnValues.exists(_ eq node))
          return Nil
        else
          AxiomaticLibraryType
      ...
      case node: PermPointerIndex[G] =>
        return Seq(SugarPermissionOperator, Pointers)
      ...
      case node: CDeclarationStatement[G] =>
        return Seq(CSpecific, UnscopedDeclaration)
      ...
      case node: CPPDeclarationStatement[G] =>
        return Seq(CPPSpecific, UnscopedDeclaration)
